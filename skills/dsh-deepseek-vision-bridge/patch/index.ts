...
export function apply(ctx: Context, config: Config): void {
  ...
  try {
    ...
    visionLog(`translator construct failed: ${error instanceof Error ? error.message : String(error)}`)
    translator = undefined // script missing: degrade per-image below
  }
  const resolveImageSupported = (): boolean => ctx.get('attachments') !== undefined
  const translateContent = (messages: Message[]): Promise<Message[]> => {
    const attachments = ctx.get('attachments')
    const nImages = messages.reduce((n, m) => n + (m.content ?? []).filter(b => b.type === 'image').length, 0)
    visionLog(`translateContent: attachments=${attachments === undefined ? 'NO' : 'YES'} translator=${translator?.label ?? 'none'} messages=${messages.length} images=${nImages}`)
    if (nImages === 0) return Promise.resolve(messages)
    const readImage = attachments === undefined
      ? undefined
      : async (ref: ImageRefLike) => {
        const stored = await attachments.readImage(ref as never)
        return { ref, data: stored.data }
      }
    const channel: VisionTranslator = translator ?? {
      label: 'placeholder',
      async translate() {
        return '(当前未配置在线识图通道，未能自动描述图片内容。)'
      },
    }
    return translateMessages(messages, readImage, channel, visionCache).then(out => {
      visionLog(`translateContent done: cache=${visionCache.size}`)
      return out as Message[]
    }, error => {
      visionLog(`translateContent FAILED: ${error instanceof Error ? error.stack ?? error.message : String(error)}`)
      throw error
    })
  }
