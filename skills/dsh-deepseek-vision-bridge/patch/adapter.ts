...
export interface DeepSeekAdapterOptions {
  ...
  /**
   * Optional async content translation applied to every request before wire
   * serialization. The DeepSeek wire is text-only; a deployment that declares
   * image input on these models routes image blocks through this hook so the
   * model still "sees" the picture as a description. Absent = the adapter
   * keeps its text-only contract and image-bearing history is rejected at
   * serialization.
   */
  translateContent?: (messages: Message[]) => Promise<Message[]>
  /**
   * Whether image input is currently advertised on this provider's models.
   * Read lazily at each model-info resolution so a seam (the attachment
   * store) that mounts after this adapter registers still enables image
   * input. Absent = no image modality declared.
   */
  resolveImageSupported?: () => boolean
}
...
export class DeepSeekAdapter extends LlmAdapter {
  ...
  override listModels(provider: string): Promise<readonly LlmModelInfo[]> {
    return Promise.resolve(this.config.options().models.map(
      model => modelInfo(provider, model, this.modelInputModalities()),
    ))
  }

  /** Whether image input is declared (true only when image content is wired). */
  ...
  override resolveModel(
    ...
    return Promise.resolve({
      ...
      // membership, so the uncatalogued fallback declares the same capability
      // as catalogued models — "unknown" here would let the host accept and
      // persist images the serializer must then handle.
      ...configured === undefined
        ? { provider, id: model, name: model, inputModalities: this.modelInputModalities() }
        : modelInfo(provider, configured, this.modelInputModalities()),
  ...
  async * stream(options: GenerateOptions): AsyncIterable<StreamChunk> {
    ...
    // Translate image-bearing content into text BEFORE serialization when the
    // deployment configured a content hook (models declaring image input on a
    // text-only wire). The hook is awaited here, outside request(), so a slow
    // vision channel never stalls the transport's watchdog accounting.
    ...
    const upstream = effectiveOptions.signal === undefined
      ? consumer.signal
      : AbortSignal.any([effectiveOptions.signal, consumer.signal])
